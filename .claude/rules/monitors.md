...
**Extension-reporting install** (`kscrash_installForExtensionReporting`): a reporter-only process that writes reports about other processes and detects no crashes of its own. It initializes the report store and pipeline, registers the given plugins, enables them, and fires `notifyPostMonitorsEnabled`; it runs no crash-detection monitors, no RunContext (so `notifyPostSystemEnable` never fires), no run id of its own (a capture loads the crashed run's), no run summaries, no console log, and no pruning. On the app side, `SendConfiguration.extensionAreas` lists the same `ExtensionConfiguration`; `sendReports(with:)` then first moves the extension's reports into the app store (no re-IDing, nanosecond ID seeds make collisions a non-event, and the move never replaces an existing file), where they stitch against their run's sidecars and send like any other report. The extension's store writes each report into a staging directory below its Reports and renames it up on completion, so the app's ingest never sees a report still being written.
