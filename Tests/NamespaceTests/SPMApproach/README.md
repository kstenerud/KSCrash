...
[`sync.sh`](sync.sh) holds a list of targets to merge from the KSCrash package, as well as a list of headers to export from it (in this case, `CrashLibA` and `CrashLibB` use `KSCrash` and `KSCrashInstallConfiguration`, so only those headers are exposed).
