...
* For these crash reports, all features are enabled, including zombie detection, memory introspection, and hang detection.
...
Generated by the legacy Deadlock monitor (now deprecated in favor of Watchdog). Caused when the main thread was locked for too long.
...
Generated by the Watchdog monitor when the main thread is unresponsive for 250ms+. If the hang resolves, the report is marked as recovered. If the OS kills the app during the hang, it becomes a fatal watchdog timeout.
...
Generated when the OS kills the app during a hang (simulated SIGKILL + 0x8badf00d). The report is reconstructed at next launch from the hang sidecar data.
